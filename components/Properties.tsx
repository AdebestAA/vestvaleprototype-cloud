...
const propertyData = [
  ...
  ...Array.from({ length: 9 }, (_, i) => ({
    ...
    tag: ['Historic Landmark', 'Penthouse', 'Equestrian Facility', 'Garden Estate'][i % 4],
    beds: 2 + (i % 3),
    baths: 1 + (i % 2),
    area: `${1200 + i * 100} sq.ft`,
    image: `https://source.unsplash.com/random/800x600?vintage+house+${i}`,
    description: 'Beautiful vintage property with a unique story and timeless design.',
  })),
];
      ...
      <div className="container mx-auto px-4">
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-gray-600 mb-4 flex items-center">
                  <FaMapMarkerAlt className="text-amber-800 mr-2" /> {property.location}
                </p>
                <div className="flex justify-between text-sm text-gray-600 mb-4">
                  <span><FaBed className="inline-block text-amber-800 mr-1" /> {property.beds} Beds</span>
                  <span><FaBath className="inline-block text-amber-800 mr-1" /> {property.baths} Baths</span>
                  <span><FaRulerCombined className="inline-block text-amber-800 mr-1" /> {property.area}</span>
                </div>
        ...
        {visibleCount < propertyData.length && (
          <div className="text-center mt-12">
            ...
            >
              Browse More Properties <FaArrowRight className="inline-block ml-2" />
            </button>
          </div>
        )}
      </div>
